...
async def get_all_products(db: AsyncSession = Depends(get_async_db)):
    all_products = await db.scalars(select(ProductModel).where(ProductModel.is_active == True))
    return all_products.all()


@router.get("/category/{category_id}", response_model=list[ProductSchema])
async def get_products_by_category(category_id: int, db: AsyncSession = Depends(get_async_db)):
    category = await db.scalars(select(CategoryModel).where(CategoryModel.id == category_id,
    ...
    db_products = products.all()
    return db_products


@router.get("/{product_id}", response_model=ProductSchema)
async def get_product(product_id: int, db: AsyncSession = Depends(get_async_db)):
    product = await db.scalars(select(ProductModel).where(ProductModel.is_active == True,
                                                          ProductModel.id == product_id))
    db_product = product.first()
